<template>
  <div
    ...
    ref="tabsListRef"
    @wheel="onWheel"
    @dragover.prevent="onTabsContainerDragOver"
    @dragleave="onTabsDragLeave"
    @drop="onTabsContainerDrop"
  >
    <template v-for="(tab, index) in tabs" :key="tab.id">
      <div
        v-if="
          (dragOverTabIndex === index && !dragOverInsertAfter) ||
          (dragOverTabIndex === index - 1 && dragOverInsertAfter)
        "
      ...
      <TabItem
        ...
        @dragstart="
          (e: DragEvent, tabId: string) => $emit('tab-dragstart', e, tabId)
        "
    ...
    <button
      ...
      :aria-label="t('startTab.defaultName')"
      @click="onAddStartTab"
    >
      <Plus :size="14" />
    </button>
  ...
  <div class="tab-more" v-if="showMore">
    <el-dropdown
      ...
      @visible-change="onMoreDropdownVisibleChange"
    >
...
function scrollToTab(tabId: string) {
  if (!tabsListRef.value) return;
  const el = tabsListRef.value.querySelector(
    `[data-tab-id="${tabId}"]`,
  ) as HTMLElement | null;
  if (el) {
    el.scrollIntoView({
