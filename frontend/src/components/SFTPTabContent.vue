  ...
  <div class="sftp-tab-content">
    <div class="panes-area">
      ...
      >
        ...
        <SFTPPathBreadcrumb
          ...
          :drives="localDrives"
          bookmark-mode="local"
          :saved-paths="settingsStore.sftpBookmarks.localPaths"
    ...
    >
      ...
      <template v-else-if="dialogType === 'chmod'">
        <div class="chmod-file-info">
          ...
          <span
            v-if="dialogItem?.owner || dialogItem?.group"
            class="chmod-ownergroup"
            >{{ dialogItem?.owner || "-" }}:{{ dialogItem?.group || "-" }}</span
          >
        </div>
    ...
    >
      <div class="editor-container">
        <div ref="editorLineNumbers" class="editor-line-numbers">
          {{ editorLineNumbersText }}
        </div>
        <textarea
          ref="editorTextarea"
      ...
      <template #footer>
        <div class="editor-footer">
          <div class="editor-footer-left">
            <el-checkbox v-model="editorWrapEnabled">{{
              t("sftp.edit.wrap")
            }}</el-checkbox>
    ...
    >
      ...
      <template #footer>
        <el-button @click="onConflictResolve('cancel')">{{
          t("sftp.dialog.cancel")
        }}</el-button>
        <el-button @click="onConflictResolve('overwrite')">{{
          t("sftp.dialog.conflictOverwrite")
        }}</el-button>
        <el-button type="primary" @click="onConflictResolve('rename')">{{
          t("sftp.dialog.conflictRename")
        }}</el-button>
      </template>
    </el-dialog>
    ...
    >
      ...
      <p
        v-if="newFileError"
        style="color: var(--el-color-danger); margin-top: 8px"
      >
        {{ newFileError }}
      </p>
      <template #footer>
        <el-button @click="newFileVisible = false">{{
          t("sftp.dialog.cancel")
        }}</el-button>
        <el-button
          type="primary"
          :loading="newFileCreating"
          @click="onNewFileCreate"
          >{{ t("sftp.dialog.confirm") }}</el-button
        >
      </template>
    </el-dialog>
  </div>
...
// validateRemotePath rejects paths containing traversal segments.
// Frontend defense-in-depth on top of the backend sanitizer.
function validateRemotePath(p: string): string | null {
  if (!p) return null;
  // Normalize backslashes; reject any literal ".." segment after split.
  const parts = p
    .replace(/\\/g, "/")
    .split("/")
    .filter((s) => s !== "" && s !== ".");
...
function decodeContent(bytes: Uint8Array, enc: Encoding): string {
  ...
  return new TextDecoder(
    enc === "utf-16le" ? "utf-16le" : enc === "utf-16be" ? "utf-16be" : "utf-8",
  ).decode(bytes);
}
...
const localClipboardCount = computed(
  () => localClipboard.value?.items.length ?? 0,
);
...
function computeVisualLines() {
  if (!editorWrapEnabled.value || !editorMirror.value || !editorTextarea.value)
    return;
...
const conflictResolve = ref<
  ((action: "overwrite" | "rename" | "cancel") => void) | null
>(null);
...
const chmodOctal = computed(() => {
  const o =
    (chmodOwnerR.value ? 4 : 0) +
    (chmodOwnerW.value ? 2 : 0) +
    (chmodOwnerX.value ? 1 : 0);
  const g =
    (chmodGroupR.value ? 4 : 0) +
    (chmodGroupW.value ? 2 : 0) +
    (chmodGroupX.value ? 1 : 0);
  const t =
    (chmodOtherR.value ? 4 : 0) +
    (chmodOtherW.value ? 2 : 0) +
    (chmodOtherX.value ? 1 : 0);
...
function applyOctal(digits: string) {
  ...
  chmodOwnerR.value = !!(nums[0] & 4);
  chmodOwnerW.value = !!(nums[0] & 2);
  chmodOwnerX.value = !!(nums[0] & 1);
  chmodGroupR.value = !!(nums[1] & 4);
  chmodGroupW.value = !!(nums[1] & 2);
  chmodGroupX.value = !!(nums[1] & 1);
  chmodOtherR.value = !!(nums[2] & 4);
  chmodOtherW.value = !!(nums[2] & 2);
  chmodOtherX.value = !!(nums[2] & 1);
}

// User typing in the octal field: strip invalid chars, apply once 3 digits are present.
function onOctalInput() {
  chmodOctalInput.value = chmodOctalInput.value
    .replace(/[^0-7]/g, "")
    .slice(0, 3);
        ...
        if (msg.type === "sftp:transfer") {
          if (msg.event === "start") {
            ...
            if (existing) {
              ...
              existing.percentage =
                existing.total > 0
                  ? Math.round((msg.progress / existing.total) * 100)
                  : 0;
              const now = Date.now();
              ...
              if (elapsed >= 0.5) {
                ...
                if (bytesPerSec > 0 && existing.total > 0) {
                  const remaining =
                    (existing.total - msg.progress) / bytesPerSec;
            ...
            if (existing) {
                ...
                st === "done"
                  ...
                  : st === "cancelled"
                    ...
                    : st === "paused"
                      ...
                      : "error";
              existing.percentage =
                existing.status === "done" ? 100 : existing.percentage;
              ...
              if (
                ...
                setTimeout(() => {
                  const idx = transferTasks.findIndex(
                    (t) => t.id === msg.taskId,
                  );
...
async function onLocalNavigate(path: string) {
  ...
  const sanitized = validateRemotePath(fullPath);
  if (!sanitized) {
    msg.error(t("sftp.invalidPath") || "Invalid local path");
    return;
  }
  fullPath = sanitized;
  const version = ++loadVersionLocal;
...
async function onRemoteNavigate(path: string) {
  ...
  const sanitized = validateRemotePath(fullPath);
  if (!sanitized) {
    msg.error(t("sftp.invalidPath") || "Invalid remote path");
    return;
  }
  fullPath = sanitized;
  const version = ++loadVersionRemote;
...
async function onUpload() {
  ...
  try {
    ...
    const fileNames = files.map(
      (fp) => fp.replace(/\\/g, "/").split("/").pop() || "upload",
    );
    const action = await checkRemoteConflicts(fileNames);
    if (action === "cancel") return;
...
async function checkRemoteConflicts(
  fileNames: string[],
): Promise<"overwrite" | "rename" | "cancel"> {
  const existingNames = remoteFiles.value.map((f) => f.name);
...
async function onPaste() {
  ...
  for (const name of items) {
    ...
    if (
      forceRename ||
      (resolveAction === "rename" && existingNames.includes(name))
    ) {
  ...
  if (pasteCancelled) {
    ...
    msg.error(
      `Copied/Moved ${success}/${items.length}, ${failed.length} failed`,
    );
  } else {
...
async function onEditFile(item: FileItem) {
  ...
  if (item.size > 500 * 1024) {
    const ok = window.confirm(
      t("sftp.edit.fileLargeWarning", { size: formatFileSize(item.size) }),
    );
    if (!ok) return;
  }
...
async function onEditorSave() {
  ...
  try {
    if (editorMode.value === "local") {
      await SftpLocalPutContent(
        sid,
        editorPath.value,
        encodeContent(
          editorContent.value,
          editorEncoding.value,
          editorLineEnding.value,
        ),
        editorEncoding.value,
      );
      onRefreshLocal();
...
async function onNewFileCreate() {
  ...
  if (!name) {
    newFileError.value = t("sftp.dialog.newFileEmpty");
    return;
  }
  if (name.includes("/") || name.includes("\\")) {
    newFileError.value = t("sftp.dialog.newFileInvalid");
    return;
  }
  const sid = panel.value?.sessionId;
  if (!sid) return;
  const isLocal = newFileMode.value === "local";
  const existingNames = (isLocal ? localFiles.value : remoteFiles.value).map(
    (f) => f.name,
  );
  ...
  switch (dialogType.value) {
    ...
    case "chmod":
      try {
        await SftpChmod(
          sid,
          joinPath(baseDir, dialogItem.value!.name),
          chmodOctal.value,
        );
        onRefreshRemote();
      } catch (e) {
    ...
    case "delete":
      for (const item of dialogItems.value) {
        ...
        try {
          ...
          console.error("delete item:", item.name, e);
        }
      }
      if (isLocal) {
        onRefreshLocal();
...
function onMkdir() {
  openDialog(
    "mkdir",
    t("sftp.dialog.mkdirTitle"),
    "",
    t("sftp.dialog.mkdirPrompt"),
  );
}
...
.editor-line-numbers {
  ...
  font-family:
    "Cascadia Code", "Fira Code", "JetBrains Mono", "Consolas", "Courier New",
    monospace;
  font-size: 14px;
  line-height: 24px;
