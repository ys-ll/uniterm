<template>
  <div class="redis-tab-content">
    <div class="redis-main">
      ...
      <div class="redis-left" :style="{ width: leftWidth + 'px' }">
        ...
        <div class="redis-toolbar">
          ...
          <button
            ...
            :aria-label="t('redis.newKey')"
            @click="onShowNewKeyDialog"
            style="flex-shrink: 0"
          >
            <Plus :size="14" />
          </button>
        ...
        <div class="redis-key-list" v-loading="loading">
          <div v-if="keys.length === 0 && !loading" class="redis-placeholder">
            {{ t("redis.noKeys") }}
          </div>
          <div
      ...
      <div class="redis-right">
        <template v-if="selectedKey">
          <div class="key-meta">
            <div class="meta-row">
              <span class="meta-label">{{ t("redis.keyName") }}</span
              ><span class="meta-value">{{ selectedKey }}</span>
            </div>
            <div class="meta-row">
              <span class="meta-label">{{ t("redis.type") }}</span
            ...
            <div class="meta-row">
              ...
              <span class="meta-value ttl-row">
                ...
                />
                <el-button size="small" @click="onSetTTL">{{
                  t("redis.setTTL")
                }}</el-button>
          ...
          <div class="redis-right-content" v-loading="keyLoading">
            ...
            <template v-else-if="selectedKeyInfo?.type === 'hash'">
              <el-table :data="hashEntries" border size="small">
                ...
                <el-table-column prop="field" :label="t('redis.field')">
                  <template #default="{ $index }">
                    <el-input
                      v-model="hashEntries[$index].field"
                      size="small"
                      type="textarea"
                      :rows="1"
                      autosize
                ...
                <el-table-column prop="value" :label="t('redis.value')">
                  <template #default="{ $index }">
                    <el-input
                      v-model="hashEntries[$index].value"
                      size="small"
                      type="textarea"
                      :rows="1"
                      autosize
                ...
                <el-table-column width="50">
                  <template #default="{ $index }">
                    <button
                      ...
                      title="Delete"
                      @click="hashEntries.splice($index, 1)"
                    >
                      <Trash2 :size="14" />
                    </button>
                  </template>
              ...
              <el-button
                ...
                @click="hashEntries.push({ field: '', value: '' })"
                ><Plus :size="14"
              /></el-button>
            </template>
                ...
                <el-table-column width="50">
                  <template #default="{ $index }">
                    <button
                      class="btn btn-ghost btn-icon btn-sm danger"
                      title="Delete"
                      @click="listEntries.splice($index, 1)"
                    >
                      <Trash2 :size="14" />
                    </button>
                  </template>
                ...
                <el-table-column width="50">
                  <template #default="{ $index }">
                    <button
                      class="btn btn-ghost btn-icon btn-sm danger"
                      title="Delete"
                      @click="setEntries.splice($index, 1)"
                    >
                      <Trash2 :size="14" />
                    </button>
                  </template>
            ...
            <template v-else-if="selectedKeyInfo?.type === 'zset'">
              <el-table :data="zsetEntries" border size="small">
                ...
                <el-table-column :label="t('redis.member')">
                  <template #default="{ $index }">
                    <el-input
                      v-model="zsetEntries[$index].member"
                      size="small"
                      type="textarea"
                      :rows="1"
                      autosize
                ...
                <el-table-column :label="t('redis.score')" width="100">
                  <template #default="{ $index }">
                    <el-input-number
                      v-model="zsetEntries[$index].score"
                      size="small"
                      controls-position="right"
                      style="width: 100%"
                    />
                ...
                <el-table-column width="50">
                  <template #default="{ $index }">
                    <button
                      class="btn btn-ghost btn-icon btn-sm danger"
                      title="Delete"
                      @click="zsetEntries.splice($index, 1)"
                    >
                      <Trash2 :size="14" />
                    </button>
                  </template>
            ...
            <div class="value-actions">
              <el-button
                ...
                @click="onSave"
                :loading="saving"
                >{{ t("redis.save") }}</el-button
              >
              <el-button size="small" @click="onRevert">{{
                t("redis.revert")
              }}</el-button>
              <el-button
                type="danger"
    ...
    >
          ...
          <el-form-item :label="t('redis.entryValue')">
            <el-table :data="newHashEntries" border size="small">
              <el-table-column prop="field" :label="t('redis.field')">
                <template #default="{ $index }"
                  ><el-input
                    v-model="newHashEntries[$index].field"
                    size="small"
                    type="textarea"
                    :rows="1"
                    autosize
              ...
              <el-table-column prop="value" :label="t('redis.value')">
                <template #default="{ $index }"
                  ><el-input
                    v-model="newHashEntries[$index].value"
                    size="small"
                    type="textarea"
                    :rows="1"
                    autosize
              ...
              <el-table-column width="50">
                <template #default="{ $index }"
                  ><button
                    class="btn btn-ghost btn-icon btn-sm danger"
                    title="Delete"
                    @click="newHashEntries.splice($index, 1)"
                  >
            ...
            <el-button
              size="small"
              style="margin-top: 4px"
              @click="newHashEntries.push({ field: '', value: '' })"
              ><Plus :size="14"
            /></el-button>
          </el-form-item>
        </template>
              ...
              <el-table-column width="26" class-name="drag-col">
                <template #default="{ $index }">
                  <span
                    class="drag-handle"
                    draggable="true"
                    @dragstart="onNewListDragStart($index)"
                    @dragover.prevent="onNewListDragOver($index)"
                    @drop="onNewListDrop($index)"
                    ><GripVertical :size="14"
                  /></span>
                </template>
              </el-table-column>
              ...
              <el-table-column width="50">
                <template #default="{ $index }"
                  ><button
                    class="btn btn-ghost btn-icon btn-sm danger"
                    title="Delete"
                    @click="newListEntries.splice($index, 1)"
                  >
                    <Trash2 :size="14" /></button
                ></template>
              </el-table-column>
              ...
              <el-table-column width="50">
                <template #default="{ $index }"
                  ><button
                    class="btn btn-ghost btn-icon btn-sm danger"
                    title="Delete"
                    @click="newSetEntries.splice($index, 1)"
                  >
                    <Trash2 :size="14" /></button
                ></template>
              </el-table-column>
          ...
          <el-form-item :label="t('redis.entryValue')">
            <el-table :data="newZSetEntries" border size="small">
              <el-table-column prop="member" :label="t('redis.member')">
                <template #default="{ $index }"
                  ><el-input
                    v-model="newZSetEntries[$index].member"
                    size="small"
                    type="textarea"
                    :rows="1"
                    autosize
              ...
              >
                <template #default="{ $index }"
                  ><el-input-number
                    v-model="newZSetEntries[$index].score"
                    size="small"
                    controls-position="right"
                    style="width: 100%"
                /></template>
              ...
              <el-table-column width="50">
                <template #default="{ $index }"
                  ><button
                    class="btn btn-ghost btn-icon btn-sm danger"
                    title="Delete"
                    @click="newZSetEntries.splice($index, 1)"
                  >
                    <Trash2 :size="14" /></button
                ></template>
              </el-table-column>
            ...
            <el-button
              size="small"
              style="margin-top: 4px"
              @click="newZSetEntries.push({ member: '', score: 0 })"
              ><Plus :size="14"
            /></el-button>
          </el-form-item>
        </template>
      ...
      <template #footer>
        <el-button @click="showNewKeyDialog = false">{{
          t("redis.cancel")
        }}</el-button>
        <el-button type="primary" @click="onCreateKey">{{
          t("redis.create")
        }}</el-button>
      </template>
    </el-dialog>
...
async function doScan(cursor: number) {
  loading.value = true;
  try {
    const result: ScanResult = await RedisScanKeys(
      props.sessionId,
      scanPattern.value,
      cursor,
      pageSize.value,
    );
    keys.value = (result.keys || []).sort((a, b) =>
...
async function onSelectKey(info: RedisKeyInfo) {
  ...
  try {
    ...
    if (fresh) {
      selectedKeyInfo.value = fresh;
      editTTL.value = fresh.ttl;
    } else {
      selectedKeyInfo.value = info;
      editTTL.value = info.ttl;
    }
  } catch (_) {
    selectedKeyInfo.value = info;
    editTTL.value = info.ttl;
  ...
  try {
    switch (info.type) {
      case "string": {
        const val = await RedisGetString(props.sessionId, info.name);
        stringValue.value = val || "";
        break;
      }
      case "hash": {
        const entries = await RedisGetHashAll(props.sessionId, info.name);
        hashEntries.value = entries || [];
        break;
      }
      case "list": {
        const items = await RedisGetListRange(
          props.sessionId,
          info.name,
          0,
          -1,
        );
        listEntries.value = items || [];
        break;
      }
      case "set": {
        const members = await RedisGetSetAll(props.sessionId, info.name);
        setEntries.value = members || [];
        break;
      }
      case "zset": {
        const members = await RedisGetSortedSetRange(
          props.sessionId,
          info.name,
          "-inf",
          "+inf",
        );
        zsetEntries.value = members || [];
        break;
      }
    }
  } catch (e: any) {
    msg.error(`Load key failed: ${e?.message || e}`);
  } finally {
...
async function onSave() {
  ...
  try {
    ...
    switch (t) {
      case "string":
        await RedisSetString(props.sessionId, key, stringValue.value);
        break;
      case "hash": {
        const current = await RedisGetHashAll(props.sessionId, key);
        const toDelete = current.filter(
          (e) => !hashEntries.value.find((h) => h.field === e.field),
        );
        if (toDelete.length > 0)
          await RedisHashDel(
            props.sessionId,
            key,
            toDelete.map((e) => e.field),
          );
        for (const e of hashEntries.value) {
          if (e.field)
            await RedisHashSet(props.sessionId, key, e.field, e.value);
        }
        break;
      }
      case "list": {
        ...
        for (let i = 0; i < existing.length; i++) {
          await RedisListPop(props.sessionId, key, "right");
        }
        if (listEntries.value.length > 0)
          await RedisListPush(props.sessionId, key, "right", listEntries.value);
        break;
      }
      case "set": {
      ...
      case "zset": {
        const current = await RedisGetSortedSetRange(
          props.sessionId,
          key,
          "-inf",
          "+inf",
        );
        const r = current.filter(
          (m) => !zsetEntries.value.find((z) => z.member === m.member),
        );
        if (r.length > 0)
          await RedisZSetRemove(
            props.sessionId,
            key,
            r.map((m) => m.member),
          );
        if (zsetEntries.value.length > 0)
          await RedisZSetAdd(props.sessionId, key, zsetEntries.value);
        break;
      }
    }
    ...
    msg.error(`Save failed: ${e?.message || e}`);
  } finally {
    saving.value = false;
  }
}
...
async function onSetTTL() {
  ...
  try {
    ...
    msg.error(`Set TTL failed: ${e?.message || e}`);
  }
}
...
async function onDeleteKey() {
  try {
    await ElMessageBox.confirm(
      t("redis.confirmDelete", { key: selectedKey.value }),
      t("redis.delete"),
      {
        confirmButtonText: t("redis.delete"),
        cancelButtonText: t("redis.cancel"),
        type: "warning",
      },
    );
  } catch (_) {
  ...
  try {
    ...
    msg.error(`Delete failed: ${e?.message || e}`);
  } finally {
...
// --- New key ---
function onShowNewKeyDialog() {
  resetNewKeyForm();
  showNewKeyDialog.value = true;
}
async function onCreateKey() {
  if (!newKeyName.value.trim()) {
    msg.warning(t("redis.keyNameRequired"));
    return;
  }
  try {
    switch (newKeyType.value) {
      case "string":
        ...
        break;
      case "hash": {
        for (const e of newHashEntries.value) {
          if (e.field)
            await RedisHashSet(
              props.sessionId,
              newKeyName.value,
        ...
        if (!newHashEntries.value.some((e) => e.field))
          await RedisHashSet(props.sessionId, newKeyName.value, "field", "");
        break;
      }
      case "list": {
        ...
        if (items.length > 0)
          ...
          await RedisListPush(props.sessionId, newKeyName.value, "right", [""]);
        break;
      }
      case "set": {
        const members = newSetEntries.value
          .map((s) => s.trim())
          .filter((s) => s);
        if (members.length > 0)
          await RedisSetAdd(props.sessionId, newKeyName.value, members);
      ...
      case "zset": {
        ...
        if (entries.length > 0)
          await RedisZSetAdd(props.sessionId, newKeyName.value, entries);
        else
          await RedisZSetAdd(props.sessionId, newKeyName.value, [
            { member: "member", score: 0 },
          ]);
        break;
      }
    }
    if (newKeyTTL.value >= 0) {
      await RedisSetKeyTTL(props.sessionId, newKeyName.value, newKeyTTL.value);
    }
    ...
    msg.error(`Create key failed: ${e?.message || e}`);
  }
}

// --- List drag reorder ---
let dragIdx = -1;
function onListDragStart(idx: number) {
  dragIdx = idx;
}
...
let newDragIdx = -1;
function onNewListDragStart(idx: number) {
  newDragIdx = idx;
}
...
function onNewListDrop(_idx: number) {
  newDragIdx = -1;
}
