  ...
  <div ref="startTabRef" class="start-tab">
    <div class="start-content" :style="contentStyle">
          ...
          >
            ...
            <span>{{
              selectedTypeFilter === "all"
                ? t("sidebar.filterAll")
                : TYPE_LABELS[selectedTypeFilter] || selectedTypeFilter
            }}</span>
          </span>
          <template #dropdown>
            <el-dropdown-menu>
      ...
      <div class="start-action-btns">
        <button
          class="start-action-btn primary"
          @click="
            emit(
              'new-connection',
              tab.viewMode === 'group' ? { groupId: tab.groupId } : undefined,
            )
          "
        >
            ...
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item
                  ...
                  :key="sh"
                  @click="
                    emit('local-terminal', sh, $event.ctrlKey || $event.metaKey)
                  "
                >
                  {{ getShellLabel(sh) }}
                </el-dropdown-item>
        ...
        <button
          ...
          @click="emit('connect-serial', $event.ctrlKey || $event.metaKey)"
        >
      ...
      <div v-if="tab.viewMode === 'group'" class="start-breadcrumb">
        <span class="link" @click="goHome">{{
          t("startTab.backToStart")
        }}</span>
        <template v-for="crumb in breadcrumbPath" :key="crumb.id">
          <span class="sep">/</span>
          <span v-if="crumb.id === tab.groupId" class="current">{{
            crumb.name
          }}</span>
          <span v-else class="link" @click="enterGroupAt(crumb.id)">{{
            crumb.name
          }}</span>
        </template>
        <span
          class="start-add-group-btn"
          @click="openNewGroupDialog"
          :title="t('conn.newGroupTitle')"
        >
      ...
      <template v-if="tab.viewMode === 'home'">
        ...
        <template v-if="recentConfigs.length > 0">
          ...
          <div class="start-cards-grid">
            <div
              ...
              class="start-card"
              :class="{
                focused: isCardFocused('recent:' + config.id),
                selected: selectedIds.has('recent:' + config.id),
              }"
            ...
            >
              <div class="start-card-top">
                <div class="start-card-icon" :class="config.type">
                  <el-icon v-if="config.type === 'ssh'"
                    ><SquareTerminal :size="28"
                  /></el-icon>
                  <el-icon v-else-if="config.type === 'telnet'"
                    ><Terminal :size="28"
                  /></el-icon>
                  <el-icon v-else-if="config.type === 'mosh'"
                    ><Zap :size="28"
                  /></el-icon>
                  <el-icon v-else-if="config.type === 'local'"
                    ><Laptop :size="28"
                  /></el-icon>
                  <el-icon v-else-if="config.type === 'serial'"
                    ><Cable :size="28"
                  /></el-icon>
                  <el-icon
                    v-else-if="config.type === 'ftp' || config.type === 'sftp'"
                    ><FolderUp :size="28"
                  /></el-icon>
                  <el-icon v-else-if="config.type === 'smb'"
                    ><HardDrive :size="28"
                  /></el-icon>
                  <el-icon v-else-if="config.type === 's3'"
                    ><Cloud :size="28"
                  /></el-icon>
                  <el-icon v-else-if="config.type === 'webdav'"
                    ><Globe :size="28"
                  /></el-icon>
                  <el-icon v-else-if="config.type === 'rdp'"
                    ><Monitor :size="28"
                  /></el-icon>
                  <el-icon v-else-if="config.type === 'vnc'"
                    ><MonitorSmartphone :size="28"
                  /></el-icon>
                  <el-icon v-else-if="config.type === 'spice'"
                    ><MonitorCloud :size="28"
                  /></el-icon>
                  <el-icon v-else-if="config.type === 'database'">
                    <DatabaseZap v-if="config.dbType === 'redis'" :size="28" />
                    <Layers
                      v-else-if="config.dbType === 'mongodb'"
                      :size="28"
                    />
                    <Database v-else :size="28" />
                  </el-icon>
                  <el-icon v-else-if="config.type === 'k8s'"
                    ><ShipWheel :size="28"
                  /></el-icon>
                  <el-icon v-else-if="config.type === 'container'"
                    ><Boxes :size="28"
                  /></el-icon>
                ...
                <div>
                  ...
                  <div class="start-card-meta">
                    {{ getCardSubtitle(config) }}
                  </div>
                </div>
              </div>
              <button
                class="card-more-btn"
                @click.stop="onCardMoreClick($event, config, 'recent:')"
        ...
        <div class="start-cards-grid">
          <div
            ...
            @contextmenu.prevent="
              onGroupContextMenu($event, group.id, group.name)
            "
          >
            <div class="start-card-top">
              <div class="start-card-icon group">
                <el-icon><Folder :size="22" /></el-icon>
              </div>
              <div>
                <div class="start-card-name">{{ group.name }}</div>
                <div class="start-card-meta">
                  {{ t("startTab.connectionsCount", { count: group.count }) }}
                </div>
              </div>
            </div>
          </div>
          ...
          >
            <div class="start-card-top">
              <div class="start-card-icon ungrouped">
                <el-icon><FolderOpen :size="22" /></el-icon>
              </div>
              <div>
                <div class="start-card-name">{{ t("conn.noGroup") }}</div>
                <div class="start-card-meta">
                  {{
                    t("startTab.connectionsCount", {
                      count: groupCards.ungroupedCount,
                    })
                  }}
                </div>
              </div>
        ...
        <template v-if="searchQuery.trim()">
          ...
          <div class="start-cards-grid">
            <div
              ...
              class="start-card"
              :class="{
                focused: isCardFocused('conn:' + config.id),
                selected: selectedIds.has('conn:' + config.id),
              }"
            ...
            >
              ...
              <button
                class="card-more-btn"
                @click.stop="onCardMoreClick($event, config)"
                :title="t('terminal.more')"
                :aria-label="t('terminal.more')"
              >
                <MoreHorizontal :size="16" />
          ...
          <div
            ...
            "
            class="start-empty-hint"
          >
            {{ t("startTab.noConnections") }}
          </div>
        </template>

        <!-- First-launch empty state: no recent, no groups, no connections -->
        <template
          v-if="
            recentConfigs.length === 0 &&
            groupCards.groups.length === 0 &&
            groupCards.ungroupedCount === 0 &&
            connectionStore.connections.length === 0
          "
        >
          <div class="start-empty-state">
            <el-icon class="start-empty-state-icon"
              ><Sparkles :size="40"
            /></el-icon>
            <div class="start-empty-state-title">
              {{ t("startTab.emptyStateTitle") }}
            </div>
            <div class="start-empty-state-subtitle">
              {{ t("startTab.emptyStateSubtitle") }}
            </div>
            <div class="start-empty-state-actions">
              <button
                class="start-empty-state-btn primary"
                @click="emit('new-connection')"
              >
                <el-icon><Plus :size="14" /></el-icon>
                {{ t("startTab.emptyStateCreateConnection") }}
              </button>
              <button
                class="start-empty-state-btn"
                @click="handleDefaultLocalTerminal"
              >
                <el-icon><Laptop :size="14" /></el-icon>
                {{ t("conn.localTerminal") }}
              </button>
              <a
                class="start-empty-state-btn ghost"
                href="https://uniterm.net/guide/en/introduction"
                target="_blank"
                rel="noopener"
              >
                <el-icon><BookOpen :size="14" /></el-icon>
                {{ t("startTab.emptyStateReadDocs") }}
              </a>
            </div>
          </div>
        </template>
      ...
      <template v-if="tab.viewMode === 'group'">
        <!-- Child groups -->
        <div v-if="groupCards.groups.length > 0" class="start-section-label">
          {{ t("startTab.groups") }}
        </div>
        ...
        <div
          v-if="filteredConnections.length > 0 || groupCards.ungroupedCount > 0"
          class="start-section-label"
        >
          {{ t("startTab.connections") }}
        </div>
        <div class="start-cards-grid">
          <div
            v-for="{ config } in filteredConnections"
      ...
      >
        <div class="start-card-top">
          <div class="start-card-icon quick">
            <el-icon><Zap :size="22" /></el-icon>
          </div>
          <div>
            <div class="start-card-name quick-name">
              {{ t("startTab.quickConnect", { host: searchQuery.trim() }) }}
            </div>
            <div class="start-card-meta">
              {{ t("startTab.quickConnectDesc") }}
            </div>
          </div>
        </div>
      </div>
      ...
      >
        <!-- Terminal -->
        <div
          v-if="contextMenuConfig && contextMenuConfig.type === 'ssh'"
          class="menu-item"
          :class="{ disabled: selectedIds.size > 1 }"
          @click="selectedIds.size <= 1 && doConnect(contextMenuConfig, $event)"
        >
          {{ t("sidebar.connectSSH") }}
        </div>
        <div
          v-if="contextMenuConfig && contextMenuConfig.type === 'telnet'"
          class="menu-item"
          :class="{ disabled: selectedIds.size > 1 }"
          @click="selectedIds.size <= 1 && doConnect(contextMenuConfig, $event)"
        >
          {{ t("sidebar.connectTelnet") }}
        </div>
        <div
          v-if="contextMenuConfig && contextMenuConfig.type === 'mosh'"
          class="menu-item"
          :class="{ disabled: selectedIds.size > 1 }"
          @click="selectedIds.size <= 1 && doConnect(contextMenuConfig, $event)"
        >
          {{ t("sidebar.connectMosh") }}
        </div>
        <div
          v-if="contextMenuConfig && contextMenuConfig.type === 'local'"
          class="menu-item"
          :class="{ disabled: selectedIds.size > 1 }"
          @click="selectedIds.size <= 1 && doConnect(contextMenuConfig, $event)"
        >
          {{ t("sidebar.connectLocal") }}
        </div>
        <div
          v-if="contextMenuConfig && contextMenuConfig.type === 'serial'"
          class="menu-item"
          :class="{ disabled: selectedIds.size > 1 }"
          @click="
            selectedIds.size <= 1 && doConnectSerial(contextMenuConfig, $event)
          "
        >
          {{ t("sidebar.connectSerial") }}
        </div>
        ...
        <div
          ...
          class="menu-item"
          :class="{ disabled: selectedIds.size > 1 }"
          @click="selectedIds.size <= 1 && doConnectSftp(contextMenuConfig)"
        >
          {{ t("sidebar.connectSftp") }}
        </div>
        <div
          v-if="contextMenuConfig && contextMenuConfig.type === 'ftp'"
          class="menu-item"
          :class="{ disabled: selectedIds.size > 1 }"
          @click="selectedIds.size <= 1 && doConnectFtp(contextMenuConfig)"
        >
          {{ t("sidebar.connectFtp") }}
        </div>
        <div
          v-if="contextMenuConfig && contextMenuConfig.type === 'smb'"
          class="menu-item"
          :class="{ disabled: selectedIds.size > 1 }"
          @click="selectedIds.size <= 1 && doConnectSmb(contextMenuConfig)"
        >
          {{ t("sidebar.connectSmb") }}
        </div>
        <div
          v-if="contextMenuConfig && contextMenuConfig.type === 's3'"
          class="menu-item"
          :class="{ disabled: selectedIds.size > 1 }"
          @click="selectedIds.size <= 1 && doConnectS3(contextMenuConfig)"
        >
          {{ t("sidebar.connectS3") }}
        </div>
        <div
          v-if="contextMenuConfig && contextMenuConfig.type === 'webdav'"
          class="menu-item"
          :class="{ disabled: selectedIds.size > 1 }"
          @click="selectedIds.size <= 1 && doConnectWebdav(contextMenuConfig)"
        >
          {{ t("sidebar.connectWebdav") }}
        </div>
        <!-- Remote Desktop -->
        <div
          v-if="contextMenuConfig && contextMenuConfig.type === 'rdp'"
          class="menu-item"
          :class="{ disabled: selectedIds.size > 1 }"
          @click="selectedIds.size <= 1 && doConnectRdp(contextMenuConfig)"
        >
          {{ t("sidebar.connectRDP") }}
        </div>
        <div
          v-if="contextMenuConfig && contextMenuConfig.type === 'vnc'"
          class="menu-item"
          :class="{ disabled: selectedIds.size > 1 }"
          @click="selectedIds.size <= 1 && doConnectVnc(contextMenuConfig)"
        >
          {{ t("sidebar.connectVNC") }}
        </div>
        <div
          v-if="contextMenuConfig && contextMenuConfig.type === 'spice'"
          class="menu-item"
          :class="{ disabled: selectedIds.size > 1 }"
          @click="selectedIds.size <= 1 && doConnectSpice(contextMenuConfig)"
        >
          {{ t("sidebar.connectSPICE") }}
        </div>
        <!-- Database & Monitor -->
        <div
          v-if="contextMenuConfig && contextMenuConfig.type === 'database'"
          class="menu-item"
          :class="{ disabled: selectedIds.size > 1 }"
          @click="selectedIds.size <= 1 && doConnectDb(contextMenuConfig)"
        >
          {{ t("db.connectDB") }}
        </div>
        <div
          v-if="contextMenuConfig && contextMenuConfig.type === 'k8s'"
          class="menu-item"
          :class="{ disabled: selectedIds.size > 1 }"
          @click="selectedIds.size <= 1 && doConnectK8s(contextMenuConfig)"
        >
          {{ t("sidebar.connectK8s") }}
        </div>
        <div
          v-if="contextMenuConfig && contextMenuConfig.type === 'container'"
          class="menu-item"
          :class="{ disabled: selectedIds.size > 1 }"
          @click="selectedIds.size <= 1 && doConnect(contextMenuConfig, $event)"
        >
          {{ t("sidebar.connectContainer") }}
        </div>
        <div
          v-if="contextMenuConfig && contextMenuConfig.type === 'ssh'"
          class="menu-item"
          :class="{ disabled: selectedIds.size > 1 }"
          @click="selectedIds.size <= 1 && doConnectMonitor(contextMenuConfig)"
        >
        ...
        <div
          class="menu-item"
          :class="{ disabled: selectedIds.size > 1 }"
          @click="selectedIds.size <= 1 && doEditConnection(contextMenuConfig)"
        >
          {{ t("sidebar.edit") }}
        </div>
        <div class="menu-item" @click="doChangeGroupBulk">
          {{ t("conn.moveTo") }}
        </div>
        <div
          class="menu-item"
          :class="{ disabled: selectedIds.size > 1 }"
          @click="selectedIds.size <= 1 && doDuplicate(contextMenuConfig)"
        >
          {{ t("sidebar.duplicate") }}
        </div>
        <div class="menu-divider" />
        <div class="menu-item danger" @click="doDeleteBulk">
          {{ t("sidebar.delete") }}
        </div>
      </div>
      ...
      <div
        ...
        @click.stop
      >
        <div class="menu-item" @click="doNewGroupFromCtx">
          {{ t("conn.newGroupTitle") }}
        </div>
        <div class="menu-item" @click="doNewConnInGroup">
          {{ t("sidebar.newConnection") }}
        </div>
        <div class="menu-divider" />
        <div class="menu-item" @click="doRenameGroup">
          {{ t("conn.renameGroup") }}
        </div>
        <div class="menu-item" @click="doChangeGroupParent">
          {{ t("conn.moveTo") }}
        </div>
        <div class="menu-divider" />
        <div class="menu-item danger" @click="doDeleteGroup">
          {{ t("conn.deleteGroup") }}
        </div>
      </div>
    </div>
    ...
    >
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="confirmRenameGroup">{{
          t("conn.save")
        }}</el-button>
      </template>
    </el-dialog>
    ...
    >
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="doAddGroup">{{
          t("conn.save")
        }}</el-button>
      </template>
    </el-dialog>
    ...
    >
      ...
      <template #footer>
        <el-button @click="showDeleteGroupDialog = false">{{
          t("conn.deleteGroupCancel")
        }}</el-button>
        <el-button type="warning" @click="confirmDeleteGroup('move-out')">{{
          t("conn.deleteGroupMoveUp")
        }}</el-button>
        <el-button
          type="danger"
          @click="confirmDeleteGroup('delete-connections')"
          >{{ t("conn.deleteGroupDeleteAll") }}</el-button
        >
      </template>
    </el-dialog>
  </div>
...
function getAllVisibleIds(): string[] {
  ...
  for (const { config } of filteredConnections.value)
    ids.push("conn:" + config.id);
  return ids;
}
...
const groupCards = computed(() => {
  ...
  function subtreeMatchCount(groupId: string): number {
    let count = connectionStore.connections.filter(
      (c) => c.groupId === groupId && matchFilter(c),
    ).length;
    for (const child of connectionStore.groups.filter(
      (cg) => cg.parentId === groupId,
    )) {
  ...
  const connFilter = isGroupView
    ? props.tab.groupId === "__ungrouped__"
      ...
      : (c: ConnectionConfig) =>
          c.groupId === props.tab.groupId && matchFilter(c)
    : (c: ConnectionConfig) => !c.groupId && matchFilter(c);
  ...
  return {
    ...
    ungroupedCount,
    isGroupView,
    currentGroupId: props.tab.groupId,
  };
});
...
function goParent() {
  ...
  const currentGroup = connectionStore.groups.find(
    (g) => g.id === props.tab.groupId,
  );
...
function updateContentWidth() {
  ...
  const cols = Math.max(
    2,
    Math.min(6, Math.floor((available + CARD_GAP) / (CARD_WIDTH + CARD_GAP))),
  );
...
const focusableItems = computed<FocusableItem[]>(() => {
  if (props.tab.viewMode === "group") {
    ...
    for (const group of groupCards.value.groups)
      items.push({ kind: "group", groupId: group.id, name: group.name });
    for (const { config } of filteredConnections.value)
      items.push({ kind: "connection" as const, config });
    if (searchQuery.value.trim()) items.push({ kind: "quick" });
    return items;
  }
  const items: FocusableItem[] = [];
  for (const config of recentConfigs.value)
    items.push({ kind: "recent", config });
  for (const group of groupCards.value.groups)
    items.push({ kind: "group", groupId: group.id, name: group.name });
  if (groupCards.value.ungroupedCount > 0)
    items.push({
      kind: "group",
  ...
  for (const { config } of filteredConnections.value)
    items.push({ kind: "connection", config });
  if (searchQuery.value.trim()) items.push({ kind: "quick" });
  return items;
});
...
function getGridColumns(): number {
  ...
  return Math.max(
    1,
    Math.floor((contentWidth.value + CARD_GAP) / (CARD_WIDTH + CARD_GAP)),
  );
}
...
function onKeydown(e: KeyboardEvent) {
  ...
  if (e.key === "ArrowRight") {
    ...
    const curEnd =
      curSection + 1 < sectionStarts.length
        ? sectionStarts[curSection + 1]
        : total;
    ...
    if (sameSectionTarget >= curStart && sameSectionTarget < curEnd) {
      ...
      if (nextSection < 0) {
        ...
        const secEnd =
          nextSection + 1 < sectionStarts.length
            ? sectionStarts[nextSection + 1]
            : total;
...
async function doAddGroup() {
  ...
  const parentId =
    newGroupParentId.value === "__none__" ? undefined : newGroupParentId.value;
  newGroupDialogName.value = "";
...
async function doDeleteGroup() {
  ...
  const connCount = connectionStore.connections.filter(
    (c) => c.groupId === g.id,
  ).length;
  const childCount = connectionStore.groups.filter(
    (cg) => cg.parentId === g.id,
  ).length;
...
function doConnect(config: ConnectionConfig, e: MouseEvent) {
  closeContextMenu();
  emit("connect", config, e.ctrlKey || e.metaKey);
}
function doConnectSerial(_config: ConnectionConfig, e: MouseEvent) {
  closeContextMenu();
  emit("connect-serial", e.ctrlKey || e.metaKey);
}
function doConnectSftp(config: ConnectionConfig) {
  closeContextMenu();
  window.dispatchEvent(new CustomEvent("app:connect-sftp", { detail: config }));
}
function doConnectMonitor(config: ConnectionConfig) {
  closeContextMenu();
  window.dispatchEvent(
    new CustomEvent("app:connect-monitor", { detail: config }),
  );
}
function doConnectRdp(config: ConnectionConfig) {
  closeContextMenu();
  window.dispatchEvent(new CustomEvent("app:connect-rdp", { detail: config }));
}
function doConnectVnc(config: ConnectionConfig) {
  closeContextMenu();
  window.dispatchEvent(new CustomEvent("app:connect-vnc", { detail: config }));
}
function doConnectSpice(config: ConnectionConfig) {
  closeContextMenu();
  window.dispatchEvent(
    new CustomEvent("app:connect-spice", { detail: config }),
  );
}
function doConnectDb(config: ConnectionConfig) {
  closeContextMenu();
  window.dispatchEvent(new CustomEvent("app:connect-db", { detail: config }));
}
function doConnectK8s(config: ConnectionConfig) {
  closeContextMenu();
  window.dispatchEvent(new CustomEvent("app:connect-k8s", { detail: config }));
}
function doConnectFtp(config: ConnectionConfig) {
  closeContextMenu();
  window.dispatchEvent(new CustomEvent("app:connect-ftp", { detail: config }));
}
function doConnectSmb(config: ConnectionConfig) {
  closeContextMenu();
  window.dispatchEvent(new CustomEvent("app:connect-smb", { detail: config }));
}
function doConnectS3(config: ConnectionConfig) {
  closeContextMenu();
  window.dispatchEvent(new CustomEvent("app:connect-s3", { detail: config }));
}
function doConnectWebdav(config: ConnectionConfig) {
  closeContextMenu();
  window.dispatchEvent(
    new CustomEvent("app:connect-webdav", { detail: config }),
  );
}
...
.start-card-icon.mosh {
  color: var(--accent);
}
.start-card-icon.local {
  color: var(--success);
}
.start-card-icon.database {
  color: var(--warning);
}
.start-card-icon.rdp,
.start-card-icon.vnc,
.start-card-icon.spice {
  color: var(--accent);
}
.start-card-icon.k8s {
  color: var(--accent);
}
.start-card-icon.serial {
  color: var(--success-dim);
}
.start-card-icon.group {
  color: var(--text-secondary);
}
.start-card-icon.ungrouped {
  color: var(--text-muted);
}
.start-card-icon.quick {
  background: var(--accent-subtle);
  color: var(--accent);
}
...
.start-empty-state-title {
  font-size: 20px;
  font-weight: 600;
  color: var(--text-primary, var(--el-text-color-primary));
  margin-bottom: 6px;
}
.start-empty-state-subtitle {
  font-size: 14px;
  margin-bottom: 24px;
  max-width: 480px;
  margin-left: auto;
...
.start-empty-state-actions {
  display: flex;
  gap: 12px;
  justify-content: center;
  flex-wrap: wrap;
}
.start-empty-state-btn {
  display: inline-flex;
  align-items: center;
  gap: 6px;
  padding: 8px 16px;
...
.start-empty-state-btn:hover {
  background: var(--bg-hover, var(--el-fill-color-light));
  border-color: var(--border-hover, var(--el-border-color-hover));
}
.start-empty-state-btn.primary {
  background: var(--el-color-primary);
  border-color: var(--el-color-primary);
  color: #fff;
}
.start-empty-state-btn.primary:hover {
  opacity: 0.9;
}
.start-empty-state-btn.ghost {
  background: transparent;
}
.empty-icon {
  font-size: 48px;
