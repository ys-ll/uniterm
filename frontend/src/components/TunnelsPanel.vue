  ...
  <div class="tunnels-panel">
    ...
    <div class="tn-toolbar">
      ...
      <el-dropdown trigger="click" placement="bottom-end" :teleported="false">
        <button
          class="tn-icon-btn"
          :title="t('tunnels.addTunnel')"
          :aria-label="t('tunnels.addTunnel')"
          @click.stop
        >
          <Plus :size="15" />
        </button>
        <template #dropdown>
          <el-dropdown-menu>
            <el-dropdown-item @click="addTunnel()">{{
              t("tunnels.addTunnel")
            }}</el-dropdown-item>
    ...
    <div class="tn-list">
      <template v-for="group in store.groups" :key="group.id">
        ...
        >
          <span class="tn-group-arrow">
            <el-icon v-if="expandedGroups.has(group.id)"
              ><ChevronDown :size="14"
            /></el-icon>
      ...
      <template
        v-if="
          ...
          store.getTunnelsByGroup(undefined).filter(matchesSearch).length > 0
        "
      >
        <div
          class="tn-group-header"
        ...
        >
          <span class="tn-group-arrow">
            <el-icon v-if="expandedGroups.has('__ungrouped__')"
              ><ChevronDown :size="14"
            /></el-icon>
            <el-icon v-else><ChevronRight :size="14" /></el-icon>
          </span>
      ...
      <div v-if="store.tunnels.length === 0" class="tn-empty">
        {{ t("tunnels.empty") }}
      </div>
    </div>
    ...
    >
      <template v-if="selectedTunnel">
        <div
          class="menu-item"
          @click="
            toggleRun(selectedTunnel!);
            closeMenu();
          "
        >
          {{
            store.statusOf(selectedTunnel!.id) === "running"
              ? t("tunnels.stop")
              : t("tunnels.start")
          }}
        </div>
        <div class="menu-divider" />
        <div class="menu-item" @click="editTunnel(selectedTunnel!.id)">
          {{ t("tunnels.editTunnel") }}
        </div>
        <div class="menu-item danger" @click="doDeleteTunnel(selectedTunnel!)">
          {{ t("tunnels.deleteTunnel") }}
        </div>
      </template>
      <template v-if="selectedGroup">
        <div class="menu-item" @click="addTunnel(selectedGroup!.id)">
          {{ t("tunnels.addTunnel") }}
        </div>
        <div class="menu-item" @click="renameGroup(selectedGroup!)">
          {{ t("tunnels.renameGroup") }}
        </div>
        <div
          class="menu-item danger"
          @click="deleteGroupDialog(selectedGroup!)"
        >
          {{ t("tunnels.deleteGroup") }}
        </div>
      </template>
    </div>
    ...
    <el-dialog
      ...
      :title="t('tunnels.deleteGroupTitle')"
      width="400px"
      :close-on-click-modal="false"
    >
      <p>{{ t("tunnels.deleteGroupDesc") }}</p>
      <div class="delete-group-actions">
        <el-button @click="doDeleteGroup(false)">{{
          t("tunnels.moveToUngrouped")
        }}</el-button>
        <el-button type="danger" @click="doDeleteGroup(true)">{{
          t("tunnels.deleteTunnels")
        }}</el-button>
      </div>
    </el-dialog>
    ...
    <el-dialog
      ...
      :title="renamingGroup ? t('tunnels.renameGroup') : t('tunnels.addGroup')"
      width="360px"
      :close-on-click-modal="false"
    >
      <el-input
        v-model="groupNameInput"
        :placeholder="t('tunnels.groupName')"
        maxlength="30"
        @keyup.enter="doSaveGroupName"
      />
      <template #footer>
        <el-button @click="groupNameDialogVisible = false">{{
          t("tunnels.cancel")
        }}</el-button>
        <el-button
          type="primary"
          :disabled="!groupNameInput.trim()"
          @click="doSaveGroupName"
          >{{ t("tunnels.save") }}</el-button
        >
      </template>
    </el-dialog>
...
function doDeleteGroup(deleteTunnels: boolean) {
  if (deletingGroup.value)
    store.deleteGroup(deletingGroup.value.id, deleteTunnels);
...
.tn-group-header:hover {
  background: var(--bg-hover);
}
.tn-group-header.drag-over {
...
.tn-context-menu .menu-item:hover {
  background: var(--bg-hover);
}
.tn-context-menu .menu-item.danger {
  color: var(--error);
}
.tn-context-menu .menu-divider {
  height: 1px;
