<template>
  <el-dialog
    append-to-body
    v-model="visible"
    :title="isEdit ? t('theme.editTitle') : t('theme.newTitle')"
    width="640px"
  ...
  >
    <div class="theme-editor">
      <div class="theme-editor-top">
        ...
        <button
          ...
          :title="t('theme.importItermcolors')"
          :aria-label="t('theme.importItermcolors')"
          @click="onImport"
        >
          <Upload :size="14" />
        </button>
        <button
          class="btn btn-ghost btn-icon btn-sm"
          :title="t('theme.exportItermcolors')"
          :aria-label="t('theme.exportItermcolors')"
          @click="onExport"
        >
          <Download :size="14" />
        </button>
      ...
      <div class="theme-preview" :style="previewStyle">
        <div class="theme-preview-line">
          <span :style="{ color: draft.colors.green }">user@host</span
          ><span :style="{ color: draft.colors.foreground }">:</span
          ><span :style="{ color: draft.colors.blue }">~/uniterm</span
          ><span :style="{ color: draft.colors.foreground }">$ ls -la</span>
        </div>
        <div class="theme-preview-line">
        ...
        <div class="theme-preview-line">
          ...
          <span :style="{ color: draft.colors.foreground }">
            5 user staff 160 Jul 3 12:00
          </span>
          <span :style="{ color: draft.colors.cyan }">.</span>
        </div>
        <div class="theme-preview-line">
          <span :style="{ color: draft.colors.foreground }"
            >-rw-r--r-- 1 user staff 512 Jul 3 12:00
          </span>
        ...
        <div class="theme-preview-line">
          <span :style="{ color: draft.colors.foreground }">$ </span>
          <span
            class="theme-preview-cursor"
            :style="{ background: draft.colors.cursor }"
          ></span>
        </div>
    ...
    <template #footer>
      <div class="theme-editor-footer">
        <el-button
          v-if="isEdit"
          class="theme-editor-delete-btn"
          @click="onDelete"
          >{{ t("theme.delete") }}</el-button
        >
...
import {
  parseItermColors,
  buildItermColors,
} from "../composables/itermcolorsParser";
import {
...
const generalKeys: (keyof TerminalThemeColors)[] = [
  "background",
  "foreground",
  "cursor",
  "selection",
];
const normalKeys: (keyof TerminalThemeColors)[] = [
  "black",
  "red",
  "green",
  "yellow",
...
const brightKeys: (keyof TerminalThemeColors)[] = [
  "brightBlack",
  "brightRed",
  "brightGreen",
  "brightYellow",
...
watch(
  ...
  (open) => {
    if (!open) return;
    const existing = settingsStore.settings.customTerminalThemes.find(
      (t) => t.id === props.sourceThemeId,
    );
    if (existing) {
      isEdit.value = true;
...
function onSave() {
  const id = isEdit.value
    ? editingId.value
    : crypto.randomUUID?.() || `custom-${Date.now()}`;
...
async function onImport() {
  let path: string;
  try {
    path = await OpenFileDialogFiltered(
      t("theme.importItermcolors"),
      "iTerm2 Color Preset",
      "*.itermcolors",
    );
  } catch {
  ...
  try {
    ...
    if (
      ...
      draft.value.name === t("theme.newDefaultName")
    ) {
      const base = path.split(/[\\/]/).pop() || "";
      draft.value.name =
        base.replace(/\.itermcolors$/i, "") || draft.value.name;
    }
...
async function onExport() {
  ...
  let path: string;
  try {
    path = await SaveFileDialogFiltered(
      t("theme.exportItermcolors"),
      defaultName,
      "iTerm2 Color Preset",
      "*.itermcolors",
    );
