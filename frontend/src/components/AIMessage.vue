<template>
  <div
    class="ai-message"
    :class="[message.role, { interrupted: isInterrupted || isTimeout }]"
  >
    ...
    <div v-if="message.skillName" class="skill-card">
      ...
      <span class="skill-card-src">{{
        message.skillSource === "auto"
          ? t("ai.skillAuto")
          : t("ai.skillExplicit")
      }}</span>
    </div>
    ...
    <div v-if="message.commandName" class="skill-card">
      ...
      <span v-if="message.commandArgs" class="skill-card-src">{{
        message.commandArgs
      }}</span>
    </div>
    ...
    <div class="content" v-if="!message.skillName && !message.commandName">
        ...
        <div v-for="tc in message.tool_calls" :key="tc.id" class="tool-pair">
          ...
          <div class="tool-box in-box">
            <div class="tool-box-header" @click="inExpanded = !inExpanded">
              ...
              <button
                class="tool-copy-btn"
                @click.stop="copyToolText(formatToolBody(tc), tc.id + '-in')"
                :title="t('ai.copy')"
              >
                <el-icon
                  ><Check v-if="copiedTool === tc.id + '-in'" :size="14" /><Copy
                    v-else
                    :size="14"
                /></el-icon>
              </button>
            ...
            <div v-show="inExpanded" class="tool-box-body">
              <pre class="tool-call-args" v-if="formatToolBody(tc)">{{
                formatToolBody(tc)
              }}</pre>
            </div>
          </div>
          ...
          <div v-if="getToolResult(tc.id)" class="tool-box out-box">
            <div class="tool-box-header" @click="outExpanded = !outExpanded">
              ...
              >
                <el-icon
                  ><Check
                    v-if="copiedTool === tc.id + '-out'"
                    :size="14" /><Copy v-else :size="14"
                /></el-icon>
              </button>
      ...
      <div v-if="pendingCmd" class="pending-tools">
        <div class="pending-tool" :class="{ dangerous: pendingCmd.dangerous }">
          <div class="tool-name">
            execute_command
            <span v-if="pendingCmd.dangerous" class="danger-badge">{{
              t("ai.dangerous")
            }}</span>
          </div>
      ...
      <div v-if="pendingQ" class="question-box">
        <div v-if="pendingQ.header" class="question-header">
          {{ pendingQ.header }}
        </div>
        ...
        <div class="question-options">
          ...
          >
            <span v-if="pendingQ.multiSelect" class="option-check">
              <el-icon v-if="selectedOptions.includes(i)"
                ><Check :size="14"
              /></el-icon>
            ...
            <span
              v-else
              class="option-radio"
              :class="{ on: selectedOptions.includes(i) }"
            ></span>
...
const isInterrupted = computed(
  () =>
    props.message.role === "tool" && props.message.content === "[INTERRUPTED]",
);
const isTimeout = computed(
  () => props.message.role === "tool" && props.message.content === "[TIMEOUT]",
);
...
const pendingCmd = computed(() =>
  isPending.value ? aiStore.pendingCommand! : null,
);

const isPendingQ = computed(
  () => aiStore.pendingQuestion?.messageId === props.message.id,
);
const pendingQ = computed(() =>
  isPendingQ.value ? aiStore.pendingQuestion! : null,
);
...
async function copyAsMarkdown() {
  try {
    ...
    setTimeout(() => {
      copyMdLabel.value = t("ai.copyMarkdown");
    }, 2000);
  } catch {
...
function renderMarkdown(text: string): string {
  ...
  html = html.replace(
    ...
    (_, lang, code) => {
      ...
      protectedBlocks.push(
        `<div class="code-block-wrapper">${header}<pre><code>${codeHtml}</code></pre></div>`,
      );
  ...
  // Markdown links — rel="noopener noreferrer" blocks window.opener hijack
  // and Referer leakage. Sanitize href in sanitizeRenderedHtml;
  // the noopener add here is a belt-and-suspenders for the markdown path.
  html = html.replace(
    /\[([^\]]+)\]\(([^)]+)\)/g,
    (_m, text, href) =>
  ...
  html = html.replace(/(?:^(&gt; ?)+.*(?:\n|$))+/gm, (block) =>
    buildNestedBlockquote(block),
  );

  // Nested unordered lists
  html = html.replace(/(?:^ {0,4}- .*(?:\n|$))+/gm, (block) =>
    buildNestedList(block, false),
  );
  // Nested ordered lists
  html = html.replace(/(?:^ {0,4}\d+\. .*(?:\n|$))+/gm, (block) =>
    buildNestedList(block, true),
  );

  // Task list checkboxes (Lucide-style SVGs)
  const TASK_CHECKED =
    '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><path d="m9 12 2 2 4-4"></path></svg>';
  ...
  html = html.replace(
    /(<li>)\[x\] /gi,
    '$1<span class="task-check checked">' + TASK_CHECKED + "</span>",
  );
  html = html.replace(
    /(<li>)\[ \] /gi,
    '$1<span class="task-check">' + TASK_UNCHECKED + "</span>",
  );
  ...
  if (tableBlocks) {
    for (const block of tableBlocks) {
      ...
      const dataLines = lines.filter(
        (line, idx) => idx !== 1 || !/^\s*[|:\-|\s]+\|\s*$/.test(line),
      );
      let tableHtml = "<table>";
  ...
  html = html.replace(
    ...
    (_, idx) => protectedBlocks[parseInt(idx)],
  );
  ...
  html = parts
    .map((part) => {
      ...
      if (
        /^<\/?(h[1-6]|pre|table|ul|ol|hr|div|p|blockquote|li|hr\s*\/?>)/i.test(
          trimmed,
        )
      ) {
        return trimmed;
  ...
  html = html.replace(
    /(<\/(h[1-6]|pre|table|ul|ol|hr|li|div|p|blockquote)>|<hr\s*\/?>)(\s*<br>)+/gi,
    "$1",
  );
  html = html.replace(
    /<br>\s*(<(h[1-6]|pre|table|ul|ol|hr|div|p|blockquote)[\s>])/gi,
    "$1",
  );
  html = html.replace(
    /<br>\s*<div class="code-block-wrapper">/gi,
    ...
    while (i < lines.length) {
      ...
      if (lineDepth === depth) {
        ...
        if (peek < lines.length) {
          ...
          const peekDepth = peekMatch
            ? peekMatch[0].split("&gt;").length - 1
            : 0;
          if (peekDepth > depth) {
...
function highlightText(html: string, query: string): string {
  ...
  let result = parts[0].replace(
    regex,
    '<mark class="ai-search-highlight">$&</mark>',
  );
  for (let i = 1; i < parts.length; i++) {
    ...
    if (gt === -1) {
      ...
      result +=
        "<" +
        parts[i].replace(regex, '<mark class="ai-search-highlight">$&</mark>');
    } else {
      ...
      result +=
        "<" +
        tag +
        text.replace(regex, '<mark class="ai-search-highlight">$&</mark>');
    }
  }
  return result;
}
...
function sanitizeRenderedHtml(html: string): string {
  ...
  // Strip javascript:/data:/vbscript: URL schemes in href/src/action/etc.
  // Exception: <img src="data:image/..."> is permitted — inline
  // images are common in chat output and the data: image/* prefix cannot
  // execute script. The exception is scoped to <img src> only.
  html = html.replace(
    ...
    (_m, prefix, _q, dq, sq, bare) => {
      ...
      // Non-allowed scheme or relative path: drop the src entirely so the
      // <img> tag degrades to a broken image rather than an attack vector.
      return prefix;
    },
  );
  html = html.replace(
